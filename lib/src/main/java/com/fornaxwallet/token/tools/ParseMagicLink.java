...
import static com.fornaxwallet.token.entity.MagicLinkInfo.mainnetMagicLinkDomain;
...
public class ParseMagicLink {
    ...
    //注意：货币链接直接处理szabo中的单位，不需要解析成wei，反之亦然
    ...
    public byte[] getSpawnableBytes(List<BigInteger> tokenIds, String contractAddress, BigInteger priceWei, long expiry) {
        try {
            //form the transaction we need to push to buy trade bytes
            //形成我们需要推送以购买交易字节的交易
