...
import com.fornaxwallet.app.entity.CovalentTransaction;
import com.fornaxwallet.app.entity.EtherscanEvent;
import com.fornaxwallet.app.entity.EtherscanTransaction;
...
import com.fornaxwallet.app.repository.entity.RealmTransfer;
...
import static com.fornaxwallet.app.repository.EthereumNetworkBase.COVALENT;
import static com.fornaxwallet.app.repository.TokenRepository.getWeb3jService;
import static com.fornaxwallet.app.repository.TokensRealmSource.databaseKey;
...
public class TransactionsNetworkClient implements TransactionsNetworkClientType {
        ...
        while (continueReading) // only SYNC_PAGECOUNT pages at a time for each check, to avoid congestion
        {
            ...
            if (myTxs.length < PAGESIZE) {
                ...
                if (lastTransaction != null)
                    storeEarliestBlockRead(instance, networkInfo.chainId, tokenAddress, Long.parseLong(lastTransaction.blockNumber));
            }
        }
    ...
    private EtherscanTransaction[] readTransactions(NetworkInfo networkInfo, String walletAddress, String tokenAddress, String firstBlock, boolean ascending, int page, int pageSize) throws JSONException {
        ...
        if (networkInfo.etherscanAPI.contains(COVALENT)) {
            return readCovalentTransactions(walletAddress, tokenAddress, networkInfo, ascending, page, pageSize);
        }
    ...
    public Single<Integer> readTransfers(String walletAddress, NetworkInfo networkInfo, TokensService svs, boolean isNFTCheck) {
        ...
        if (nftCheck && svs.openSeaUpdateInProgress(networkInfo.chainId)) {
            return Single.fromCallable(() -> 0);
        } //don't allow simultaneous NFT checking
    ...
    private void writeTokens(String walletAddress, NetworkInfo networkInfo, EtherscanEvent[] events, TokensService svs) {
        ...
        for (String contract : eventMap.keySet()) {
            ...
            if (tokenDecimal == -1 && (token == null ||
                ...
                newToken = true;
                if (BuildConfig.DEBUG)
                    Log.d(TAG, "Discover NFT: " + ev0.tokenName + " (" + ev0.tokenSymbol + ")");
                ...
                newToken = true;
                if (BuildConfig.DEBUG)
                    Log.d(TAG, "Discover ERC20: " + ev0.tokenName + " (" + ev0.tokenSymbol + ")");
            } else if (token == null) {
                svs.addUnknownTokenToCheck(new ContractAddress(networkInfo.chainId, ev0.contractAddress));
                if (BuildConfig.DEBUG)
                    Log.d(TAG, "Discover unknown: " + ev0.tokenName + " (" + ev0.tokenSymbol + ")");
                continue;
            }
    ...
    private String readNextTxBatch(String walletAddress, NetworkInfo networkInfo, long currentBlock, String queryType) {
        ...
        if (networkInfo.etherscanAPI.contains(COVALENT)) {
            return readCovalentTransfers(walletAddress, networkInfo, currentBlock, queryType);
        }
        String result = "0";
