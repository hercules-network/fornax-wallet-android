...
import com.fornaxwallet.app.entity.TransactionType;
...
public class TransactionsService {
    ...
    private void checkTransfers() {
        ...
        if (tokensService.getCurrentAddress() == null || filters.size() == 0 ||
                (eventFetch != null && !eventFetch.isDisposed())) {
            return;
        } //skip check if the service isn't set up or if a current check is in progress
    ...
    private void handleMoveCheck(long chainId, boolean isNFT) {
        ...
        if (isNFT)
            tokensService.checkingChain(0); //this flags to TokensService that the check is complete. This avoids race condition
        eventFetch = null;
    }
    ...
    private void onUpdateTransactions(Transaction[] transactions, Token token) {
        ...
        if (BuildConfig.DEBUG)
            Log.d(TAG, "Queried for " + token.tokenInfo.name + " : " + transactions.length + " Network transactions");
    ...
    private void stopAllChainUpdate() {
        ...
        if (pendingTransactionCheckCycle != null && !pendingTransactionCheckCycle.isDisposed()) {
            pendingTransactionCheckCycle.dispose();
        }
        if (tokenTransferCheckCycle != null && !tokenTransferCheckCycle.isDisposed()) {
            tokenTransferCheckCycle.dispose();
        }
