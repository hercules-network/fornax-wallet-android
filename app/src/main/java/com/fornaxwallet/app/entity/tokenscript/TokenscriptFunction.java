...
import com.fornaxwallet.token.entity.As;
...
public abstract class TokenscriptFunction {
    ...
    public Function generateTransactionFunction(Token token, BigInteger tokenId, TokenDefinition definition, FunctionDefinition function, AttributeInterface attrIf) {
        ...
        if (tokenId.bitCount() > 256)
            tokenId = tokenId.or(BigInteger.ONE.shiftLeft(256).subtract(BigInteger.ONE)); //truncate tokenId too large
    ...
    private String fetchArgValue(Token token, TokenscriptElement element, Attribute attr, BigInteger tokenId, TokenDefinition definition, AttributeInterface attrIf) {
        if (attr.userInput) {
            if (!TextUtils.isEmpty(element.value))
                return element.value; //nullify user input if value is not set
            ...
            try {
                if (attr.userInput) {
                    ...
                    if (BuildConfig.DEBUG)
                        System.out.println("ATTR: " + attr.label + " : " + attr.name + " : " + attr.getSyntaxVal(attr.toString(val)));
            ...
            switch (attr.as) {
                ...
                case Bytes:
                    ...
                    if (inputBytes.length <= 32) {
                        ...
                        convertedValue = com.fornaxwallet.token.tools.Numeric.toHexString(inputBytes);
                    }
                    break;
                case e18:
