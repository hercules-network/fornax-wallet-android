...
import static com.fornaxwallet.app.util.BalanceUtils.gweiToWei;
...
public class GasPriceSpread {
    ...
    public int setupGasSpeeds(Context ctx, List<GasSpeed> gasSpeeds, int currentGasSpeedIndex) {
        ...
        if (standard.compareTo(BigInteger.ZERO) > 0) {
            ...
            if (!hasRapid)
                txTime = GasPriceSpread.FAST_SECONDS; //for non mainnet chains, assume standard tx time is 1 minute
