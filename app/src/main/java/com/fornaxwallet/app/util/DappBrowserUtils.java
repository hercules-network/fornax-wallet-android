...
import static com.fornaxwallet.app.repository.EthereumNetworkBase.isWithinHomePage;
...
public class DappBrowserUtils {
    ...
    public static List<DApp> getMyDapps(Context context) {
        ...
        if (TextUtils.isEmpty(myDappsJson))
            myDappsJson = loadFromPrefsLegacy(context, "my_dapps", MY_DAPPS_FILE); //load legacy
    ...
    public static List<DApp> getBrowserHistory(Context context) {
        ...
        if (TextUtils.isEmpty(historyJson))
            blankPrefEntry(context, C.DAPP_BROWSER_HISTORY); //blank legacy
