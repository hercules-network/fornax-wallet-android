...
import static com.fornaxwallet.app.entity.MagicLinkParcel.generateReverseTradeData;
import static com.fornaxwallet.token.tools.ParseMagicLink.currencyLink;
import static com.fornaxwallet.token.tools.ParseMagicLink.customizable;
import static com.fornaxwallet.token.tools.ParseMagicLink.normal;
import static com.fornaxwallet.token.tools.ParseMagicLink.spawnable;
import static com.fornaxwallet.token.tools.ParseMagicLink.unassigned;
...
public class ImportTokenViewModel extends BaseViewModel {
    ...
    public void importThroughFeemaster(String url) {
        try {
            initParser();
            MagicLinkData order = parser.parseUniversalLink(universalImportLink);
            disposable = fornaxWalletService.handleFeemasterImport(url, wallet.getValue(), network.getValue().chainId, order)
                    .subscribeOn(Schedulers.io())
                    .observeOn(AndroidSchedulers.mainThread())
    ...
    public void checkFeemaster(String feemasterServer) {
        disposable = fornaxWalletService.checkFeemasterService(feemasterServer, network.getValue().chainId, importOrder.contractAddress)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
